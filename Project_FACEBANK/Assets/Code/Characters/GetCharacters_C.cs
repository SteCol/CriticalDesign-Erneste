...
public class GetCharacters_C : MonoBehaviour
{
    ...
    public List<string> selfiesPaths;
    ...
    public void GetPaths()
    {
        ...
        foreach (string p in allPaths)
        {
            ...
            if (p.Contains("Selfie_") && !p.Contains("meta")) //Get the profile pics.
            {
                selfiesPaths.Add(p);
                Debug4("Found selfie at" + p);
            }
        }
    }
    ...
    public void GetFiles()
    {
        ...
        foreach (string p in selfiesPaths)   //Get the character Profile pics

        {
            ...
            output = output.Substring(0, output.Length - 4); //Removes '.psd'
                                                             //print(output);
            selfies.Add((Sprite)Resources.Load("," + output, typeof(Sprite))); //Takes the paths generated by 'output' and gets the profilePic_.psd files.
        }
    }
            ...
            if (getPeriods)
            {
                ...
                foreach (Period p in characters[c].periods) {
                    foreach (StatusUpdate su in p.statusUpdates) {
                        foreach (Sprite s in selfies)
                        {
                            if (su.content.Contains(s.name))
                            {
                                su.selfie = s;
                                break;
                            }
    ...
    public void GetStatusUpdate(string[] line, int c, int q, int p)
    {
        ...
        if (line[q].Contains("+SU"))
        {
            ...
            characters[c].periods[p].statusUpdates.Add(new StatusUpdate(line[q])); //add su
